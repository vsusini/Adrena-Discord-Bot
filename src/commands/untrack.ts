...
export async function handleUntrackCommand(
  ...
  try {
    ...
    const selectMenu = new StringSelectMenuBuilder()
      ...
      .addOptions(
        userPositions.map((pos) => ({
          label: `${
            pos.symbol
          } ${pos.side.toUpperCase()} - ${formatters.walletAddress(
            pos.wallet
          )}`,
    ...
    collector.on("collect", async (selectInteraction) => {
      ...
      const success = tracker.removeUserFromPosition(
        positionId,
        interaction.user.id
      );
    ...
    await interaction.editReply(
      "Error fetching tracked positions. Try again later."
    );
  }
}
